...
import { retry, tap, throwError, catchError } from 'rxjs';
...
export class CreateStaffComponent {
  ...
  get phoneNumber() {
    return this.staffForm.get('phone_number');
  }
  ...
  createStaff() {
    if (this.password?.value === this.confirmPassword?.value) {
      ...
      this.apiService
        .post('staff', JSON.stringify(this.staffForm.value), options)
        .pipe(
          tap((response: any) => {
            if (response.status === 'success' && response.data.length > 0) {
              this.localStorageService.unVerifiedAdmin = false;
